...
from semi_seg.epocher import FullEpocher, TrainEpocher, EvalEpocher, InferenceEpocher
...
class SemiTrainer(Trainer):
    ...
    def __init__(self, *, num_iter: int = config["Iterations"]["num_iter"], model: nn.Module, labeled_loader: T_loader, unlabeled_loader: T_loader,
    ...
    def _eval_epoch(self, *, loader: T_loader, **kwargs) -> Tuple[EpochResultDict, float]:
        evaler = EvalEpocher(self._num_iter, self._model, val_loader=loader, sup_criterion=self._sup_criterion,
...
class FullTrainer(SemiTrainer):
    ...
    def __init__(self, *,num_iter: int, model: nn.Module, labeled_loader: T_loader, val_loader: T_loader,
    ...
    def _run_epoch(self, *args, **kwargs) -> EpochResultDict:
        trainer = FullEpocher(num_iter = self._num_iter, model = self._model, optimizer = self._optimizer, labeled_loader = self._labeled_loader,
